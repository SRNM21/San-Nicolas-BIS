...
class PDF extends FPDF 
{
    ...
    function FancyTable($header, $data) 
    {
        ...
        $w = array(41, 41, 28, 41, 28, 28, 28, 27);
        ...
        foreach($data as $row) 
        {
            ...
            $this->Cell($w[1],6,$row['address'],'LR',0,'C',$fill); 
            $this->Cell($w[2],6,$row['purok'],'LR',0,'C',$fill); 
            $this->Cell($w[3],6,$row['email'],'LR',0,'C',$fill); 
            $this->Cell($w[4],6,$row['occupation'],'LR',0,'C',$fill); 
            $this->Cell($w[5],6,$row['educational'],'LR',0,'C',$fill); 
...
$pdf = new PDF('L');
$pdf->SetTitle("SPOUSE REPORT");
$pdf->SetMargins(6, 20, 25);
$pdf->AliasNbPages();
$pdf->AddPage();
$pdf->Watermark();

$header = array('Fullname', 'Address', 'Purok' ,'Email', 'Occupation', 'Education', 'Birthdate', 'Civil Status');
$data = array();
