...
const doc = $(document)
const win = $(window)
const famHeadTable = $('.fam-head-tbl-body')
...
function populateTable(data)
{
    ...
    data.forEach(item => {
        ...
        const ageCell               = $('<td>').text(item['age'])
        const civilStatusCell       = $('<td>').text(item['civil_status'])
        const dateOfInterviewCell   = $('<td>').text(item['date_of_interview'])
    
        const detailsBtn    = $('<a href="family-head/info?id=' + id + '" class="data-util-btn more-details-btn">Details</a>').data('id', id)
        ...
        tableRow.append(lastNameCell, firstNameCell, middleNameCell, purokCell, ageCell, buttonCell, civilStatusCell, dateOfInterviewCell, buttonCell)
