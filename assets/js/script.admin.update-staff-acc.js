...
const resetPassContainer = $('.reset-pass-wrapper')
...
{  
    ...
    resetPassContainer.css('display', 'flex');
    resetPassBtn.remove();
})
...
{  
    ...
    if (!validateUsername($('#username'), $('.un-err')))
    {
        return;   
    }

    if (resetPassBtn === undefined)
    {
        if (!validatePassword($('#password'), $('.cps-err'), $('#conf-password')))
        {
            return;
        }
    }
